  ...
  return (
    ...
    <div className="min-h-screen bg-[#F8FAFC] text-zinc-800 p-6 flex flex-col gap-30 font-sans relative overflow-hidden">
      ...
      <div className="w-full max-w-5xl grid grid-cols-1 lg:grid-cols-12 gap-12 items-center relative z-10">
        ...
        <div className="lg:col-span-7 flex flex-col space-y-6 items-center">
          
          <div className="w-full max-w-[440px] bg-white border-2 border-slate-200/80 p-2.5 rounded-2xl shadow-sm grid grid-cols-3 gap-2">
            
            <button
              type="button"
              onClick={() => { setActiveStyle('classic'); setSubmittedData(p => ({ ...p, style: 'classic' })) }}
              className={`flex flex-col items-center gap-1.5 py-2.5 rounded-xl border font-mono text-[10px] font-bold transition-all ${
                activeStyle === 'classic' 
                ? 'bg-[#0055BF] text-white border-[#0055BF] shadow-sm' 
          ...
          <div className="perspective-1000 p-4 w-full flex justify-center items-center">
            <motion.div
              ref={cardRef}
              onMouseMove={handleMouseMove}
              onMouseLeave={() => { x.set(0); y.set(0) }}
              style={{ rotateX, rotateY, transformStyle: 'preserve-3d' }}
              className={`w-[440px] h-[268px] rounded-2xl ${currentTheme.bg} p-6 shadow-2xl ${currentTheme.glow} transition-all duration-700 ease-out flex flex-col justify-between border-2 border-white/30 relative overflow-hidden group cursor-grab active:cursor-grabbing select-none`}
              >
              {submittedData.style === 'classic' && (
                <div className="absolute inset-0 grid grid-cols-6 gap-y-8 p-6 opacity-25 pointer-events-none z-0">
                ...
                <>
                  ...
                  <div className="my-1 flex items-center justify-between z-10" style={{ transform: 'translateZ(55px)' }}>
                    <div className="w-16 h-16 relative bg-white/10 rounded-xl border border-white/20 p-2 flex items-center justify-center backdrop-blur-xs">
                      <img src={Logo} alt='Logo' className='w-full h-full object-contain' priority/>
                    </div>
